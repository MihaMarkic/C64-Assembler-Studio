...
public abstract class Project: NotifiableObject
{
    ...
    /// </summary>
    public string LibDirs { get; set; } = string.Empty;

    /// <summary>
    /// Returns <see cref="LibDirs"/> as <see cref="ImmutableArray{String}"/>.
    /// </summary>
    public ImmutableArray<string> LibDirArray =>
        [..LibDirs.Split(';').Select(d => d.ConvertsDirectorySeparators().Trim()).Where(d => !string.IsNullOrWhiteSpace(d))];
}